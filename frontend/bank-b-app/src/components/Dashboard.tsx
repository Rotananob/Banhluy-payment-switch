...
export const Dashboard: React.FC<{ onBalanceChange: () => void }> = ({
  ...
  // Transfer state
  const [toAccount, setToAccount] = useState('BB-00000002');
  const [transferAmount, setTransferAmount] = useState<number | ''>(50);
  const [transferDesc, setTransferDesc] = useState('Dinner share');
  ...
  const handleIntraTransfer = async (e: React.FormEvent) => {
    ...
    if (!toAccount || !transferAmount || Number(transferAmount) <= 0) {
      toast.error('Enter a valid Bank B account number and amount');
      return;
    }
      ...
      <div className="md:hidden mobile-bottom-nav">
        <button
          ...
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-xs font-semibold transition ${
            activeTab === 'overview' ? 'text-purple-400 bg-purple-500/15' : 'text-slate-400'
          }`}
        >
          <CreditCard size={20} />
        ...
        <button
          ...
          onClick={() => setActiveTab('transfer')}
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-xs font-semibold transition ${
            activeTab === 'transfer' ? 'text-purple-400 bg-purple-500/15' : 'text-slate-400'
          }`}
        >
          <Send size={20} />
        ...
        <button
          ...
          onClick={() => setActiveTab('receive')}
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-xs font-semibold transition ${
            activeTab === 'receive' ? 'text-purple-400 bg-purple-500/15' : 'text-slate-400'
          }`}
        >
          <QrCode size={20} />
        ...
        <button
          ...
          onClick={() => setActiveTab('scan')}
          className={`flex flex-col items-center justify-center p-2 rounded-xl text-xs font-semibold transition ${
            activeTab === 'scan' ? 'text-purple-400 bg-purple-500/15' : 'text-slate-400'
          }`}
        >
          <ScanLine size={20} />
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="glass-card p-6 bg-gradient-to-br from-slate-900/90 to-slate-950 border-white/10">
                <div className="flex items-center justify-between mb-4">
                  <div className="text-xs uppercase font-semibold text-purple-400 flex items-center gap-2">
                    <Sparkles size={14} />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    type="button"
                    onClick={() => setActiveTab('transfer')}
                    className="p-4 rounded-2xl bg-purple-600/15 hover:bg-purple-600/25 border border-purple-500/30 text-white font-semibold flex flex-col items-start gap-2 transition"
                  >
            ...
            <div className="lg:col-span-7 glass-card p-6 sm:p-7">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
                ...
                <button
                  ...
                  className="text-xs text-purple-400 hover:text-purple-300 flex items-center gap-1.5 font-semibold bg-purple-500/10 px-3 py-1.5 rounded-xl border border-purple-500/20 w-fit"
                >
                  <RefreshCw
                    size={14}
          ...
          >
            <div className="text-center mb-6">
              <h2 className="text-xl sm:text-2xl font-bold text-white">
                Intra-Bank Transfer (Bank B)
              </h2>
              <p className="text-sm text-slate-400 mt-1">
                Instant ACID transfers inside Bank B with $0.00 fee.
              </p>
            </div>

            <div className="mb-6 bg-purple-500/10 border border-purple-500/30 rounded-2xl p-4 flex items-center justify-between">
              <div className="text-xs text-purple-200">
                Test Account Number:{' '}
                <span className="font-mono font-bold">BB-00000002</span>
              </div>
              <button
                type="button"
                onClick={() => setToAccount('BB-00000002')}
                className="px-3 py-1.5 rounded-xl bg-purple-600 text-white text-xs font-semibold hover:bg-purple-500 transition"
              >
                Fill BB-00000002
              </button>
            </div>
            ...
            <form onSubmit={handleIntraTransfer} className="space-y-5">
                ...
                <div className="flex flex-wrap gap-2 mt-2.5">
                  {[10, 25, 50, 100, 500].map((amt) => (
                    <button
                      ...
                      className="px-3 py-1 rounded-lg bg-white/5 hover:bg-purple-500/20 border border-white/10 text-xs font-mono text-slate-300 font-semibold transition"
                    >
                      +${amt}
                    </button>
                  ))}
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Dinner share / Invoice #102"
        ...
        {activeTab === 'receive' && (
          ...
          >
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-slate-400 mt-1">
                EMVCo-compliant Bakong KHQR payload. Scan from Bank A to test cross-bank clearing!
              </p>
            </div>
            ...
            {generatedQr && (
              <div className="bg-slate-950/90 border border-purple-500/30 rounded-3xl p-6 text-center shadow-2xl">
        ...
        {activeTab === 'scan' && (
          ...
          >
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-slate-400 mt-1">
                Paste an EMVCo TLV payload string to pay Bank A across the Bakong / Central Switch clearing house.
              </p>
            </div>
            ...
            <div className="space-y-5">
              <div>
                ...
                <textarea
                  ...
                  placeholder="00020101021226360007BANHLUY0111BA-000000010206BANK_A..."
              ...
              {decodedKhqr && (
                <form onSubmit={handlePayKhqr} className="mt-6 bg-slate-900/95 border border-purple-500/40 rounded-3xl p-6 space-y-5 shadow-2xl">
                  ...
                  <div className="bg-purple-500/10 border border-purple-500/30 rounded-2xl p-4 text-xs text-purple-200">
