  ...
  if (!mounted || (initialLoading && accessToken)) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 p-6 text-center">
        <div className="w-14 h-14 rounded-2xl bg-purple-600/20 border border-purple-500/40 flex items-center justify-center text-purple-400 mb-4 animate-bounce">
          <Database size={28} />
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <footer className="border-t border-white/5 py-6 text-center text-xs text-slate-500 font-mono">
        BanhLuy Dual-Bank Interoperability Ecosystem — Bank B Cyber Amethyst System © {new Date().getFullYear()}
      </footer>
    </div>
  );
}
