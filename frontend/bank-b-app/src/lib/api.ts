...
export const getProfileApi = async () => {
  const res = await apiClient.get('/bank-b/profile');
  return res.data;
};

export const getTransactionsApi = async (page = 1, limit = 20): Promise<TransactionsResponse> => {
  const res = await apiClient.get<TransactionsResponse>(`/bank-b/transactions?page=${page}&limit=${limit}`);
  return res.data;
};
...
export const transferIntraBankApi = async (
  ...
  const res = await apiClient.post('/bank-b/transfer', {
    toAccountNumber,
    amount,
    description,
  });
