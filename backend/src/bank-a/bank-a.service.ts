...
export class BankAService {
  private readonly logger = new Logger(BankAService.name);
  ...
  async intraTransfer(senderAccountId: string, dto: TransferDto) {
    ...
    // If cross-bank, reject — must go through central-switch
    if (receiverAccount.bankType !== BankType.BANK_A) {
    ...
    // ACID transaction: debit sender, credit receiver, create records
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const transfer = await tx.transfer.create({
        data: {
          ...
          senderBankType: BankType.BANK_A,
          receiverBankType: BankType.BANK_A,
          amountCents,
          feeCents: 0n,
          isCrossBank: false,
